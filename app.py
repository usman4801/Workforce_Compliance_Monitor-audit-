...
def percentage(numerator, denominator):
    if denominator in [0, None] or pd.isna(denominator): return 0.0
...
def process_attendance_data(dates_tuple, warehouse, manual_str, exclude_str, roster_map):
    ...
    def get_hours(row):
        cid = row["Clean_ID"]
        if cid in manual_list: return "7 Hours"
    ...
    def analyze(row):
        ...
        if total_punches == 0: return pd.Series([0, target, "00:00", "OK", "Absent", "Clean"])
        if total_punches == 1: return pd.Series([1, target, "N/A", "Error", "Single Scan Only", "Mispunch"])
        ...
        if total_punches % 2 == 0:
            if min_mins <= eff_mins <= max_mins: return pd.Series([total_punches, target, hr_str, "OK", "Complete Within Window", "Clean"])
            elif eff_mins < min_mins: return pd.Series([total_punches, target, hr_str, "Error", "Under Time", "Defaulter Hours"])
            else: return pd.Series([total_punches, target, hr_str, "Error", "Over Time", "Defaulter Hours"])
    ...
    for idx, col in enumerate(p_cols):
        ...
        p_clean[clean_col_name] = a_df[col].apply(lambda x: parse_time(x).strftime("%H:%M") if parse_time(x) else "")
    ...
    return pd.concat([basic_info, analyzed, p_clean], axis=1), missing_files
...
def detect_status_from_row(row):
    possible_columns = ["status", "attendance status", "leave type", "leave", "absence type", "reason", "remarks", "attendance", "type"]
